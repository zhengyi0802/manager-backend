...
@section('content')
    ...
    <form action="{{ route('appadvertisings.update',$appadvertising->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-12">
                <div name="project_group" id="project_group" class="form-group">
                    ...
                    <select id="proj_id" name="proj_id" >
                          <option value="0" {{ ($appadvertising->proj_id == 0) ? "selected" : null }}>------</option>
                        @foreach($projects as $project)
                           <option value="{{ $project->id }}" {{ ($appadvertising->proj_id == $project->id) ? "selected" : null }}>{{ $project->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
